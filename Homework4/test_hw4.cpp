...
template <template<typename, typename, typename> typename T, template<typename, typename> typename TD>
double map_test() {
    double score = 0.0;
    using int_double_map = TD<int, double>;
...
int main() {
    ...
    if constexpr(interface_part1 && interface_part2) {
        score += map_test<student_std::unordered_map, student_std::unordered_map>();
    }
    else {
