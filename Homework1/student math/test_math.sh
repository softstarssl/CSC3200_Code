...
if "$CXX" $CXXFLAGS -c student_math.cpp -o /dev/null >/dev/null 2>&1; then
  CPP_OK=1
fi
...
if "$CXX" $CXXFLAGS test_math.cpp student_math.cpp -o test_math >/dev/null 2>&1; then
  OUT="$(./test_math 2>/dev/null)"
  echo "$OUT"
  echo ""
  echo ""
  NUM="$(printf '%s\n' "$OUT" | awk '
