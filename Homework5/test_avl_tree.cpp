...
int main() {
    ...
    bool find_correctness = true; //#
    {
        std::mt19937 rng{0};
        // 配置更大的测试规模
        const int INSERT_OPS      = 20000;           // 插入操作次数
        const int ERASE_OPS       = INSERT_OPS / 2;  // 删除操作次数（可按需调整）
        const int VALUE_MAX       = INSERT_OPS * 20; // 拉大取值范围，减少重复
        const int VALIDITY_PERIOD = 100;             // 降频校验：每 100 次做一次全树校验
        std::uniform_int_distribution<> dist(0, VALUE_MAX);
