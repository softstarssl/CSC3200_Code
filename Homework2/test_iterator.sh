...
if "$CXX" test_iterator_interface.cpp -o interface_test; then
  CPP_OK=1
  rm interface_test
fi
...
echo ""
echo ""
echo "Run test_iterator"

trap 'rm -f test_iterator' EXIT HUP INT TERM

if "$CXX" test_iterator.cpp -o test_iterator; then
  OUT="$(./test_iterator)"
  echo "$OUT"
  echo ""
  echo ""
  NUM="$(printf '%s\n' "$OUT" | awk '
    /^test_iterator.cpp subtotal:/ {
